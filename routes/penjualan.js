...
router.post('/upload', requireLogin, requireAdmin, upload.fields([
  ...
  if (!fileOrder) return res.status(400).json({ error: 'File Sales Order (sale.order) wajib diupload' });
  ...
  try {
    ...
    const wajibA = ['Order Reference', 'Order Date', 'Customer', 'Salesperson', 'Total', 'Invoice Status'];
    ...
    // Key join pakai 'Order Reference' + 'Customer' (bukan Order Reference saja),
    // karena pada data Odoo nyata 'Order Reference' TIDAK selalu unik antar order
    ...
    for (let i = 2; i <= wsA.rowCount; i++) {
      ...
      const difaktur       = invoiceStatus !== 'To Invoice' ? 1 : 0;
      ...
      orders.push({
        ...
        periode: toPeriode(orderDateRaw),
        customer,
        salesperson,
        area_kerja: area,
        total,
    ...
    // 'Order Reference' pada data Odoo tidak selalu unik, jadi idempotensi re-upload
    // dijaga dengan cara: hapus dulu semua baris periode yang ada di file ini,
    // baru insert ulang dari isi file (bukan upsert per order_reference).
    const deleteByPeriode = db.prepare('DELETE FROM penjualan_order WHERE periode=?');
    const insertOrder = db.prepare(`
      INSERT INTO penjualan_order (order_reference, order_date, periode, customer, salesperson, area_kerja, total, invoice_status, laba_kotor, difaktur, uploaded_by, uploaded_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now','localtime'))
    `);

    const aggStmt = db.prepare(`
      SELECT SUM(total) as penjualan,
             SUM(CASE WHEN difaktur=1 THEN total ELSE 0 END) as difaktur,
             SUM(laba_kotor) as laba_kotor
      FROM penjualan_order WHERE area_kerja=? AND periode=?
    `);
    ...
    orders.forEach(o => { if (o.periode) periodeTerpengaruh.add(o.periode); });

    const findAreasByPeriode = db.prepare('SELECT DISTINCT area_kerja FROM penjualan_order WHERE periode=?');

    const txn = db.transaction(() => {
      // Tangkap dulu area yang datanya ada SEBELUM dihapus, supaya area yang di
      // upload baru ini sudah tidak punya order lagi tetap ikut direkalkulasi (jadi 0),
      // bukan menyisakan angka lama yang basi di sales_target.
      periodeTerpengaruh.forEach(p => {
        findAreasByPeriode.all(p).forEach(r => areaPeriodePairs.set(r.area_kerja + '::' + p, { area: r.area_kerja, periode: p }));
      });

      periodeTerpengaruh.forEach(p => deleteByPeriode.run(p));

      orders.forEach(o => {
        insertOrder.run(o.order_reference, o.order_date, o.periode, o.customer, o.salesperson, o.area_kerja, o.total, o.invoice_status, o.laba_kotor, o.difaktur, req.session.user.id);
        if (o.periode) areaPeriodePairs.set(o.area_kerja + '::' + o.periode, { area: o.area_kerja, periode: o.periode });
      });
...
router.get('/', requireLogin, requireAdmin, (req, res) => {
  ...
  if (PUSAT_ROLES.includes(user.role)) {
    ...
    rows = db.prepare('SELECT * FROM penjualan_order WHERE periode=? AND area_kerja=? ORDER BY order_reference').all(periode, myArea);
  }
  res.json(rows);
});
...
router.get('/summary', requireLogin, requireAdmin, (req, res) => {
  ...
  if (PUSAT_ROLES.includes(user.role)) {
    ...
    const myArea = (db.prepare('SELECT area_kerja FROM users WHERE id=?').get(user.id) || {}).area_kerja || '';
    rows = db.prepare('SELECT area_kerja, penjualan as total_penjualan, difaktur, laba_kotor, target FROM sales_target WHERE periode=? AND area_kerja=?').all(periode, myArea);
  }
  res.json(rows);
});
